...
<details class = "section-head">

<summary>LAB MATERIALS</summary>
...
<details class = "section-head">
<summary>Plots with {ggplot2}</summary>
# Plots with {ggplot2}
...
```{r include = FALSE}
qqplot <- data.frame(qqnorm(regdat$weight)[2:1])

names(qqplot) <- c("Observed Values", "Z Values")
```
...
<details class = "section-head">
<summary>Creating new variables</summary>
...
ggplot(skewed, aes(x = skewed_var)) + 
...
We can use a natural log transformation to correct for that skew:
[[@joe what kind of language would we like to use here?]]
...
<details class = "section-head">
<summary>Checking normality and collinearity</summary>
# Checking normality and collinearity

To better understand the measures of skewness and kurtosis, please watch this video. 

<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/HnMGKsupF8Q?start=160" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

<aside>

[Here is a link](https://www.analyticsvidhya.com/blog/2021/05/shape-of-data-skewness-and-kurtosis/) for those who would prefer to read about it.
...
The Methods and formulas section in the [Stata Documentation](https://www.stata.com/manuals/rsktest.pdf) mentions a Royston adjustment to the D'Agostino omnibus test of normality. We may need to hard-code this calculation in order to get a test statistic similar to that rendered in the Stata output for `sktest ln_wt`.  
...
<aside>
The __Pearson__ is the parametric test. Both __Spearman__ and __Kendall__ refer to rank-based (non-parametric) measures of association for when our data is non-normal.
</aside>
...
<details class = "section-head">
<summary>Linear Regression Models</summary>
...
<details class = "section-head">
<summary>Plotting line of best fit</summary>
...
1. Plot, then have {ggplot2} calculate line of best fit

2. Build model, then plot using model object

<aside>
[Here](https://stats.idre.ucla.edu/r/faq/how-can-i-explore-different-smooths-in-ggplot2/) is a highly useful tutorial for how to plot different models in {ggplot2}. It includes splines, Loess, and multiple variatons on quadratics.
</aside>
...
The data above is __heteroskedastilisticexpialidocious.__
...
<details class = "section-head">
<summary>Nested models (task 13, but fancy)</summary>

# Nested models (task 13, but fancy)

## Intro

This section is drawn from Hadley Wickham's ["R for Data Science"](https://r4ds.had.co.nz/many-models.html). Honestly, Hadley does a much better job. I'm mostly using this to learn it myself. 
...
```{r}
knitr::kable(head(iris_by_species$data[[1]]))
```
...
```{r}
resids <- unnest(iris_by_species, resids) %>% select(-data, -model)
