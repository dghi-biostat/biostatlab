...
title: "Lab 02 (707)"
description: | 
    __Lab 02 Due:__ March 18, 2022 by 14:00 ET (__Note:__ There are no written questions to turn in for Lab 2.)
    <br></br>
...
Next, use `predict.lm()` to generate predicted values and their confidence intervals using our model object:

```{r echo = TRUE}
model_predict <- predict.lm(model, interval = "confidence")
```
...
This has created a new variable, `education_bord5`, that contains six categories, one for each unique combination of `education` and `bord5` (since 3 levels * 2 levels = 6 unique combinations):
