...
kable(
  head(
    kenya %>%
      select(education_bord5, "education_bord5_No primary.1-4 in Birth Order":"education_bord5_Post-primary.5+ in Birth Order")
  )
)
```
...
```{r echo = TRUE, eval = FALSE}
m1 <- lm(weight~height, data=regdat)
```
...
```{r echo = TRUE, eval = FALSE}
m1 <- glm(weight ~ height, data = regdat, family = 'guassian'(link = 'identity'))
```
...
```{r echo = TRUE}
# fit model
m1 <- glm(death ~ education_1 + education_2, 
...
Once we have exponentiated our coefficients, we can interpret them
...
[[@potato - yes, agreeed. let's hold off for Week1/lab2 and come back to this discussion next week!]]
