...
description: |
    __Due:__ November 12, 2021 by 14:00 ET
date: "`r Sys.Date()`"
...
* [__Excel document with Tables 1, 2, and 3__](assignments/Lab3_Tables.xlsx)
...
__INSTRUCTIONS:__ Fill in `Table 1` with the descriptive statistics for maternal BMI (`mbmi`) and maternal age (`mage`), overall and by levels of child mortality within 5 years (`death`).  

This table also includes statistical measures (T-test and Kruskal-Wallis) of the association between the outcome, `death`, and maternal BMI and age (i.e., comparing the distributions of maternal BMI and maternal age for the children who died versus those who were alive).  
...
### Overall summary statistics by variable
...
__Note__: Table 1 asks for stratified counts of `death` as they apply to `mbmi` and `mage`. If you look closely, this means that we need counts of `death` that exclude missing values for our two continuous variables. `skim()` does not give you raw counts of the variable `death` that account for those missing values.

Use `table()` or a pipe, `group_by()` and `count()` to get the counts of children in each group of `death`. Then use the column of your skim output, `n_missing`, to subtract missing values from groups 


### T-tests and Kruskal-Wallis tests

T-tests and one of their nonparametric cousins, Kruskal-Wallis, can be used to compare means between sub-populations, with the null hypothesis (H0) being that all means are equal.
...
<aside> Many functions in R can perform a task like generating tables, while computing a p-value as a matter of course (see [Task 4](https://dghi-biostat.github.io/biostatlab/lab_3.html#task-4-table-2] below). </aside>
...
Our formal question might look like this: "Does mean character height differ by a character's gender?"
...
[[@Liz Feel free to cut this next part with the density plots if it's not helpful. Just trying to think of ways to get students to understand how these tests apply to real data]]
...
<aside>The T-test asks: assuming the two curves are normally distributed and given our sample size, is the distance between the two red lines (the means of each curve) sufficiently large to say that the two curves are *statistically* different? </aside>
...
[[@Liz I made the decision to use {tableone} for this task because it is an extremely useful tool for tabular analysis and generating Table 1 summary statistics (hence the name). I found it immensely useful in my work this summer.

It's also efficient. Students will have to take time to learn how to use this very convenient and powerful tool, but the trade-off is that they will avoid the tedious work of computing the stratified frequency counts, percentages, and tests of significance for each variable individually. 

Alternatively, we could cut this part and opt for simpler instructions that simply tell students to do 2x2 tables for each variable, and then run chisq.test() and fisher.exact() for each variable individually. But I thought shaking things up and giving them a useful skill might be nice. Your call!
]]
...
The first, "All subjects" requires the __overall__ frequency counts and percentages by the categorical variables of interest. These are the same values that you generated for Table 1 in both labs 1 and 2.
...
After that, we'll use `print()` to view the output generated by those two different object.
...
<aside> Setting `showAllLevels` to `FALSE` will leave out the reference category for each variable. </aside>
...
#### Print stratified table with Chi-squared test

Whereas the simple table prints overall frequency counts and percentages, our stratified table will take the stratifying variable, and perform a chi-squared test for differences between the various groups [[@Liz would it be good to add "when compared with the overall distributions of size and pnc"?]] :
...
When we look at our [decision tree for hypothesis testing](TODO), we might notice that we need to use a Fisher's Exact test when our data doesn't meet the assumption of normality necessary for a Chi-squared test of significance. 
...
Although in this particular example we draw the same conclusions from the 
Fisher's exact test as we do from the Chi-square test, assuming normality with insufficient data can lead to misleading results.

<aside>
There are other arguments for {tableone} that allow you to specify exactly which variables are nonnormal, and therefore perform the nonparametric test on only those variables. Similar steps apply for analysis of continuous variables (you could even try using the continuous variables from Task 3!)
</aside>
...
__INSTRUCTIONS:__ Fill in `Table 3` with frequency counts overall and by levels of child mortality within 5 years and with the stratum-specific risk of death (these are the same frequency count values, *N*, as in Table 2). Then use `epi.2by2()` to compute the risk difference, risk ratio and the odds ratio of death comparing levels for each of the 5 variables to the reference group.  

![Table 3](images/lab3_table3.png)

<aside> We will use `mAssoc()` in the package {epiAssist} to generate these values. This function is a helper function, and runs using functions available in {epiR}` </aside>

The reference group for each variable is noted in the spreadsheet ("REF").  For the two 3-level variables, calculate risk differences and ratios for each non-referent level relative to the reference group.  
...
RD, RR, and OR are what are known as "measures of association" because they are used to compare outcomes between levels of a categorical variable. In epidemiology, we refer to the baseline variable as the "Reference variable" and the comparator variable as the "Index variable". 
...
<aside> \[\frac{index\ \ variable}{reference\ \ variable}\] </aside>
...
<aside> If we set `units = 100`, our attributable risk in the exposed (risk difference) would be interpretable as a difference of *x* number of outcomes per 100 people in the index group when compared to the referent group. Since our population unit is set to 1, this number is simply interpreted as an individual's risk difference of experiencing the outcome if they are in the index category of the exposure.</aside>
...
Now that you've seen the code for `bord5` as an example, if you're paying close attention, you may have started to wonder, "But what about variables like `education`, when there's more than one index category?"
...
__INSTRUCTIONS:__ Generate boxplots of maternal BMI (`mbmi`) and maternal age (`magec`), stratified by levels of child mortality (`death`).

<aside>[Click here](https://dghi-biostat.github.io/biostatlab/help_ggplot2.html#generate-a-stratified-boxplot-by-adding-a-y-aesthetic) for more help with boxplots using {ggplot2} </aside>
