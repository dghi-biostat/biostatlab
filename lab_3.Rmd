...
As an example, we might use the derived variables in our kenya dataset for `size` and `pnc`. We need to put the variable names in quotes, separating them by commas within a vector. Then we need to assign the vector a name so that we can call it later. Here, we've named the vector `variables`. 
...
We may also want to stratify our results by a second variable, like `bord5`. If we inspect gender, we notice that it's a *character* type variable. This means we'll need to convert it to a factor using `factorVars= `

```{r eval = FALSE, echo = TRUE}
names(kenya)

#> [1] "caseid"    "v008"      "v011"      "v025"      "v437"      "v438"      "v463z"    
#> [8] "bord"      "b3"        "b4"        "b5"        "b7"        "m2n"       "m18"      
#> [15] "m19"       "s109"      "mage"      "magec"     "bord5"     "male"      "mweight"  
#> [22] "mheight"   "mbmi"      "size"      "belowavg"  "pnc"       "rural"     "education"
#> [29] "death"     "time"
```
...
#### Stratified table with Chi-square test

Whereas the simple table prints overall frequency counts and percentages, our stratified table will take the stratifying variable, and perform a chi-square test to for differences between the various groups:
...
As the default for categorical variables, the {tableone} `print()` function performs a chi-square test on the difference of proportions between the different strata for `bord5`. 
...
When we look at our [decision tree for hypothesis testing](TODO), we notice that we need to use a Fisher's Exact test when our data doesn't meet the assumption of normality necessary for a Chi-square test of significance. 
...
__*Attrib risk in the exposed*__: The risk difference. `mAssoc()` takes another argument, `units = `, which by default is set to 1. 
...
Here's an example of what we are looking for, using the `starwars` dataset. We've adjusted the dimensions a bit to accomodate our lone outlier. Care to guess who it is?
...
<aside> ![Jabba](images/TODOjabba.png) </aside>
