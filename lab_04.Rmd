...
zp <- cox.zph(cox_adj, transform = function(time)log(time))
zp
```
...
plot(zp[1])
abline(0,0, col = 2)
abline(h = cox_adj$coefficients[1], col = 3, lwd = 2, lty = 2)
```
...
4. The link below is to a tutorial for a function called timeSplitter. Could be an interesting substitute task. You'll see that the current solution in R for estimating a time-varying effect is a little bit "black box". timeSplitter() could be a more hands-on type of task:
...
95\%\  CI &=& exp[\beta_1 + \beta_2*T +/- (1.96 * SE(\beta_1 + \beta_2*T))]
\end{eqnarray}
$$


## Part 2: Calculating incidence rates and related measures of association
