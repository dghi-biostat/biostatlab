...
[[@potato - yes, i think so but it's not all that halpful since the equation is so simple yet so opaque. if you have time to copy/paste from old lab resource into new pdf that woudl be great. otherwise, i can work on it soon. i don't think it's critical for now though]]
...
cox_bord5_ed <- coxph(Surv(time, death) ~ bord5 + 
      ...
      data = kenya)
```
[[@potato, can you please add code using factor() as well.]]
...
[[@potato - thanks for this. the answers are very similar. different software handle event date ties differently which may explain the discepancy but i think since it's so minor, we can ignore. but i should look to see how R handles ties. otherwise - the interaction term is simply a linear interaction so i think you are doing this correctly (same as Stata)!]]
...
Note that the exposure time is explicitly specified in the GLM options using `offset()`. We must use the natural log of `time` to generate the appropriate estimates since our model is fit on the log scale.
