...
They pipe can be interpeted as signifying __"and then"__. In the above example, the pipe tells the R console to take the dataset `birds`, __and then__ filter by `state` and `height`, __and then__ group by `sky`, __and then__ count the observations in each group.
